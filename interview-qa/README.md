# Q&A - STAR Methodology

[Back to Main Index](../README.md)

---

**300 questions** across Beginner, Intermediate, and Advanced levels using the **STAR methodology** (Situation, Task, Action, Result) plus an **AI Vision** for each — showing how the solution could be enriched with AI/ML.

All examples are from real-world **US secondary mortgage market** scenarios (Fannie Mae, Freddie Mac, Ginnie Mae, Intex, CoreLogic).

---

## Beginner (100 Questions)

### [SQL Server - Beginner (Q1-50)](sql-server-beginner.md)
JOINs, indexes, data types, stored procedures, views, normalization, transactions, NULL handling, string/date functions, constraints, backup, permissions, catalog views, math functions

### [Snowflake - Beginner (Q1-50)](snowflake-beginner.md)
Architecture, warehouses, stages, COPY INTO, Time Travel, RBAC, micro-partitions, VARIANT, cloning, QUALIFY, PUT/LIST, MERGE, transactions, ARRAY/OBJECT, SHOW/DESCRIBE, billing

---

## Intermediate (100 Questions)

### [SQL Server - Intermediate (Q1-50)](sql-server-intermediate.md)
Window functions, CTEs, MERGE, CROSS APPLY, PIVOT, dynamic SQL, execution plans, covering indexes, JSON, query hints, temporal tables, columnstore, CDC, compression, Extended Events

### [Snowflake - Intermediate (Q1-50)](snowflake-intermediate.md)
Streams, Tasks, Snowpipe, clustering keys, Dynamic Tables, FLATTEN, masking, row access policies, Snowpark, Marketplace, MATCH_RECOGNIZE, geospatial, replication, UDTFs, Account Usage

---

## Advanced - SQL Server (50 Questions)

### [Part 1: Architecture and Internals (Q1-10)](sql-server-part1.md)
Buffer pool, tempdb, partitioning, I/O optimization, memory configuration, filegroups

### [Part 2: Query Optimization and Indexing (Q11-20)](sql-server-part2.md)
Execution plans, parameter sniffing, Query Store, statistics, window functions, Extended Events

### [Part 3: Concurrency, Transactions and HA/DR (Q21-30)](sql-server-part3.md)
Deadlocks, SNAPSHOT isolation, Always On AG, CDC, distributed transactions, backup/recovery

### [Part 4: Security, ETL and Administration (Q31-40)](sql-server-part4.md)
Encryption, SSIS, Row-Level Security, Resource Governor, auditing, data archival

### [Part 5: Advanced Features and Modern SQL (Q41-50)](sql-server-part5.md)
Columnstore, In-Memory OLTP, temporal tables, graph DB, JSON, Azure migration, IQP

---

## Advanced - Snowflake (50 Questions)

### [Part 1: Architecture, Compute and Storage (Q1-10)](snowflake-part1.md)
Multi-cluster warehouses, micro-partitions, clustering keys, cost governance, editions

### [Part 2: Data Loading, Semi-Structured and SQL Features (Q11-20)](snowflake-part2.md)
Snowpipe, FLATTEN, MERGE SCD2, Streams/Tasks, Dynamic Tables, recursive CTEs

### [Part 3: Performance and Optimization (Q21-30)](snowflake-part3.md)
Search Optimization, materialized views, query profiling, warehouse sizing, Time Travel costs

### [Part 4: Security, Data Sharing and Governance (Q31-40)](snowflake-part4.md)
Dynamic masking, Secure Data Sharing, Row Access Policies, RBAC, audit, compliance

### [Part 5: Advanced Features, Integration and AI (Q41-50)](snowflake-part5.md)
Snowpark, Iceberg Tables, Cortex AI, dbt, ML FORECAST, CI/CD, migration, AI vision
